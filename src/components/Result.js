...
const Result = ({selected, results, qizLength}) => {
  ...
  if(selected.length === qizLength) {
    ...
    console.log(results[score%results.length].img);
    showResults = <div className="question">
      <img src={`assets/${results[score%results.length].img}`} alt={results[score%results.length].superhero} height="250" width="400"></img>
    </div>;
  }
